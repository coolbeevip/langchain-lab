...
# fix0001-add a default metadata extractor to response content_type zhanglei


def _metadata_extractor(raw_html: str, url: str, content_type: str) -> dict:  # fix0001
    """Extract metadata from raw html using BeautifulSoup."""
    metadata = {"source": url, "content_type": content_type}  # fix0001
    try:
    ...
    **Security Note**: This loader is a crawler that will start crawling
    ...
    def __init__(
        ...
        *,
        ...
        self.metadata_extractor = metadata_extractor if metadata_extractor is not None else _metadata_extractor
        self.exclude_dirs = exclude_dirs if exclude_dirs is not None else ()

        if any(url.startswith(exclude_dir) for exclude_dir in self.exclude_dirs):
            raise ValueError(f"Base url is included in exclude_dirs. Received base_url: {url} and " f"exclude_dirs: {self.exclude_dirs}")
    ...
    def _get_child_links_recursive(self, url: str, visited: Set[str], *, depth: int = 0) -> Iterator[Document]:
        ...
        try:
            ...
            if self.continue_on_failure:
                logger.warning(f"Unable to load from {url}. Received error {e} of type " f"{e.__class__.__name__}")
                return
            else:
                raise e
        ...
        if content:
            yield Document(
                ...
                metadata=self.metadata_extractor(response.text, url, response.headers["Content-Type"]),  # fix0001
            )
    ...
    async def _async_get_child_links_recursive(
        ...
        *,
        ...
        if not self.use_async or not self._lock:
            raise ValueError("Async functions forbidden when not initialized with `use_async`")

        try:
            import aiohttp
        except ImportError:
            raise ImportError("The aiohttp package is required for the RecursiveUrlLoader. " "Please install it with `pip install aiohttp`.")
        if depth >= self.max_depth:
            return []
        ...
        try:
            ...
            if close_session:
                await session.close()
            if self.continue_on_failure:
                logger.warning(f"Unable to load {url}. Received error {e} of type " f"{e.__class__.__name__}")
                return []
            else:
                raise e
        results = []
        ...
        if content:
            results.append(
                Document(
                    page_content=content,
                    metadata=self.metadata_extractor(text, url, response.headers["Content-Type"]),  # fix0001
                )
            )
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        visited: Set[str] = set()
        if self.use_async:
            results = asyncio.run(self._async_get_child_links_recursive(self.url, visited))
