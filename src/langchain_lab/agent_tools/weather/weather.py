...
class WeatherInput(BaseModel):
    city_name: str = Field(description="应该是一个中文的城市名称，这个名称不应该包含省份或区县，例如陕西省西安市碑林区或陕西西安碑林，只需要西安")
...
def weather(city_name: str) -> str:
    ...
    if response.status_code == 200:
        ...
        if data["status"] == 1 or data["status"] == "1":
            return f"为您查询，{real_city_name}当前的天气是{data['lives'][0]['weather']}，" f"气温{data['lives'][0]['temperature']}度，空气湿度{data['lives'][0]['humidity']}。"
